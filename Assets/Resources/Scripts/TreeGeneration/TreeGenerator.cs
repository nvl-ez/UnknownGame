...
public class TreeGenerator
{
    ...
    public void placeTree(Vector3 pos, Vector3 up) {
        gen();
        tree.transform.position = pos;
        tree.transform.up = up;
    }
    ...
    Mesh GenBranch(GameObject tree, int basePolygon, Vector3[] startVertices, Vector2[] startUv, float thickness, float floorHeight, int floorNumber, Vector3 startingPos, Vector3 startingDirection, float angularOffset, float distorsionCone, float roughness, float branchDensity, int recursionLevel)
    {
        ...
        mesh.RecalculateNormals();
        {
            ...
            if (leavesMesh != null) {
                Vector3[] trVertices = (Vector3[])(leavesMesh.vertices.Clone());
                int[] leavesTriangles = (int[])(leavesMesh.triangles.Clone());

                for (int i = 0; i < leavesMesh.vertices.Length; ++i) {
                ...
                trLeavesMesh.SetNormals((Vector3[])(leavesMesh.normals.Clone()));
                trLeavesMesh.SetTangents((Vector4[])(leavesMesh.tangents.Clone()));
                trLeavesMesh.uv = (Vector2[])(leavesMesh.uv.Clone());
            }
            Mesh res = new Mesh();
            res.subMeshCount = 2;
